    ...
    if not app_state.character_selected:
        ...
        if character_options:
            ...
            if st.button("Load Character"):
                ...
                app_state.profile = store_service.get_character_profile(app_state.character_id)
                app_state.chc_profile = store_service.get_chc_profile(app_state.character_id)
        ...
        if st.button("Merge Selected Profiles"):
            if len(selected_profiles_info) < 2:
                ...
                with st.spinner("Merging profiles..."):
                    ...
                    merged_profile = combine_character_profiles(selected_profiles, app_state.user_id)
                    ...
                    if not description:
                        ...
                        with st.spinner("Generating profile... This may take a moment."):
                            if assessment_type == "RIASEC":
                                profile = generate_character_profile(description, "gemini-2.5-pro", app_state.user_id)
                            else:
                                profile = generate_hexa3d_profile(description, "gemini-2.5-pro", app_state.user_id)
                ...
                if st.button("Generate CHC Profile"):
                    if not description:
                        ...
                        with st.spinner("Generating CHC profile... This may take a moment."):
                            chc_profile = generate_chc_profile(description, "gemini-2.5-pro", app_state.user_id)
                            chc_profile.character_id = str(uuid.uuid4())
                            store_service.save_chc_profile(chc_profile, app_state.user_id)
            ...
            if app_state.profile is not None:
                ...
                if app_state.profile.tcc_program:
                    ...
                    with st.spinner("Generating TCC program... This may take a moment."):
                        tcc_program = generate_tcc_program(app_state.profile, "gemini-2.5-pro")
                        app_state.tcc_program = tcc_program
                        store_service.update_profile_with_tcc_program(app_state.profile.character_id, tcc_program)
                    ...
                    for j, activity in enumerate(module.activities):
                        ...
                        if f"{i}-{j}" in app_state.detailed_sessions:
                            with st.expander("View Detailed Session Plan"):
                                st.markdown(app_state.detailed_sessions[f"{i}-{j}"], unsafe_allow_html=True)
...
if app_state.user_id:
    ...
    for p in all_profiles:
        ...
        if char_name not in characters_history:
            characters_history[char_name] = {"name": p['character_name'], "profiles": []}
            ...
            for profile_entry in char_data['profiles']:
                ...
                if st.button(display_text, key=f"view-{profile_entry['character_id']}"):
                    ...
                    if profile_entry['profile_type'] == "RIASEC":
                        app_state.profile = store_service.get_character_profile(profile_entry['character_id'])
                    elif profile_entry['profile_type'] == "CHC":
                        app_state.chc_profile = store_service.get_chc_profile(profile_entry['character_id'])

                    if app_state.profile is not None and app_state.profile.tcc_program:
                        app_state.tcc_program = app_state.profile.tcc_program
