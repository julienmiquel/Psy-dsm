"""
This module provides a service layer for interacting with Google Cloud Datastore.
It handles saving, retrieving, and updating character profiles, CHC profiles,
and user profiles.
"""
...
def get_datastore_client():
    ...
    return datastore.Client(
        project=os.getenv("GOOGLE_CLOUD_PROJECT"), database=db_name
    )
...
def save_profile(profile: CharacterProfile, user_id: str):
    ...
    pii_entity.update({"character_name": profile.character_name})
...
def get_character_profile(character_id: str):
    ...
    if entity:
        ...
        if "tcc_program" in profile_data and profile_data["tcc_program"]:
            profile_data["tcc_program"] = TCCProgram.model_validate_json(
                profile_data["tcc_program"]
            )
...
def get_chc_profile(character_id: str):
    """Retrieivs a CHCProfile for a given character_id."""
    client = get_datastore_client()
...
def get_user_profile(user_id: str):
    """Retrieivs a user profile from Datastore."""
    client = get_datastore_client()
