...
class DiagnosisSpecifier(BaseModel):
    """Specific details for a diagnosis, like severity."""
    specifier_type: str
    value: str

class DiagnosisEntry(BaseModel):
    """Represents a single diagnosis entry based on DSM-5."""
    disorder_name: str
    dsm_category: str
    criteria_met: List[str] = Field(
        default_factory=list, description="Specific DSM-5 criteria codes/text met"
    )
    specifiers: List[DiagnosisSpecifier] = Field(default_factory=list)
    dsm_code: Optional[str] = Field(
        None, description="The official DSM-5 code, e.g., '301.7'"
    )
    functional_impairment: Optional[str] = Field(
        None, description="How the disorder impairs the character's life"
    )
    diagnostic_note: Optional[str] = Field(
        None, description="Clinical notes or differential diagnosis"
    )

class HollandCode(BaseModel):
    """A single score for a Holland Code theme."""
...
class HollandCodeAssessment(BaseModel):
    ...
    riasec_scores: List[HollandCode] = Field(
        default_factory=list, description="List of RIASEC scores."
    )
...
class CharacterProfile(BaseModel):
    """The complete psychological profile of a character."""
    character_name: str
    ...
    overall_assessment_summary: Optional[str] = Field(
        None, description="A brief summary of the clinical assessment"
    )
    holland_code_assessment: Optional[HollandCodeAssessment] = Field(
        None, description="Holland Code (RIASEC) assessment results."
    )
